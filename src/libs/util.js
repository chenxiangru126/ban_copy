...
util.ajax.interceptors.request.use(function (config) {
    // console.log(config)
    //在请求发出之前进行一些操作
    if(config.url == "/admin/authCopyright/save.do" || config.url == "/admin/copyrightTemp/getId.do"){
        config.headers=get_usr_info();//判断make页面中不让它显示转圈加载
        // Indicator.close()
    }else{
        config.headers=get_usr_info();
        Indicator.open({ spinnerType: 'fading-circle'}); //闪狗
    }
